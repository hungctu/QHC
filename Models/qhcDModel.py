...
class qhcDModel:
    ...
    def createCode(self, code):
        try:
            query = """INSERT INTO `qhc_d` (`code`) VALUES (%s)"""
...
#LAY CODE KHI QUET QR
    ...
    def getCode(self):
        try:
            query = """select id,code,unit from qhc_d where status=1 LIMIT 1"""
    ...
    def getNearestCode(self,last_code_time,device_name):
        try:
            query="""SELECT code,unit from qhc_d c join device_code dc on c.id=dc.code_id join device d on dc.device_id=d.id 
    ...
    def updateCodeStatus(self, id):
        try:
            query = """UPDATE `qhc_d` SET `status`=0, `get_code_at`=%s WHERE id=%s"""
    ...
    def checkCountCode(self):
        try:
            query = """select count(id) as nums from qhc_d where status=1 is_used=0 LIMIT 1"""
            result = self.db.fetch_one(query, ())
            if result['nums'] == 0:
                return False
            return True
    ...
    def getCodeForUser(self,device_id):
        try:
            ...
            # print(type(id))
            # print(type(ip_id))
            # cic = icm.createIpCode(id,'QHC_D',ip_id)
            icm = DeviceCodeModel()
            print(type(id))
    ...
    def checkCode(self, code):
        try:
            query = """select count(id) as nums from qhc_d where code = %s"""
            result = self.db.fetch_one(query, (code,))
            if result['nums'] == 0:
                return False
            return True
        except Exception as e:
            print(f"Error when check Code form qhc_d: {e}")
            return False

    def checkCodeIsUsed(self,code):
        try:
            query = """select is_used from qhc_d where code = %s"""
            result = self.db.fetch_one(query, (code,))
            if result['is_used'] == 1:
                return False
            return True
    ...
    def updateCodeIsUsed(self, code, is_used):
        try:
            query = """UPDATE `qhc_d` SET `is_used`=%s, `used_code_at`=%s WHERE code=%s"""
            # Lấy thời gian hiện tại
            current_time = datetime.now()
    ...
    def SendToken(self, address):
        try:
            hash = self.pol.send_transaction(address, 1)
            if not hash['status']:
                return {
                    'status': False,
                    'error': hash['error']
    ...
    def addCW(self, code, id):
        try:
            query = """INSERT INTO `get_qhcd`(`code`, `wallet_id`) VALUES (%s,%s)"""
    ...
    def transaction(self, code, address):
        ...
        try:
            ...
            at = transfer.addTransfer(walletId, code, 1, hash['hash'])
            if at == False:
    ...
    def getCodeInfo(self,code):
        query = """select id,unit from qhc_d where code = %s"""
        result = self.db.fetch_one(query,(code,))
        return result
    ...
    def getCodeUnit(self, code):
        try:
            query = """select unit from qhc_d where code = %s"""
            result = self.db.fetch_one(query, (code,))
