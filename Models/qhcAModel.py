...
class qhcAModel:
    ...
    def getCode(self):
        try:
            query = """select id,code,unit from qhc_a where status=1 LIMIT 1"""
    ...
    def updateCodeStatus(self, id):
        try:
            query = """UPDATE `qhc_a` SET `status`=0, `get_code_at`=%s WHERE id=%s"""
    ...
    def getNearestCode(self,last_code_time,device_name):
        try:
            query="""SELECT code,unit from qhc_a c join device_code dc on c.id=dc.code_id join device d on dc.device_id=d.id 
    ...
    def createCode(self, code):
        try:
            query = """INSERT INTO `qhc_a` (`code`) VALUES (%s)"""
    ...
    def checkCode(self,code):
        try:
            query = """select count(id) as nums from qhc_a where code = %s"""
    ...
    def checkCodeIsUsed(self,code):
        try:
            query = """select is_used from qhc_a where code = %s"""
            result = self.db.fetch_one(query, (code,))
            print(result['is_used'])
            if result['is_used'] == 1:
                return False
            return True
        except Exception as e:
    ...
    def updateCodeIsUsed(self,code,is_used):
        try:
            query = """UPDATE `qhc_a` SET `is_used`=%s, `used_code_at`=%s WHERE code=%s"""
            # Lấy thời gian hiện tại
            current_time = datetime.now()
    ...
    def SendToken(self,address):
        try:
            hash = self.pol.send_transaction(address, 0.001)
            if not hash['status']:
                return {
                    'status': False,
                    'error': hash['error']
    ...
    def addCW(self,code,id):
        try:
            query="""INSERT INTO `get_qhca`(`code`, `wallet_id`) VALUES (%s,%s)"""
    ...
    def transaction(self,code,address):
        ...
        try:
            ...
            at = transfer.addTransfer(walletId,code, 0.001, hash['hash'])
            if at==False:
    ...
    def getCodeInfo(self,code):
        query = """select id,unit from qhc_a where code = %s"""
        result = self.db.fetch_one(query,(code,))
        return result
    ...
    def getAllCodeCanUse(self):
        try:
            query = """select id,code,unit from qhc_a where is_used=0"""
    ...
    def getCodeUnit(self, code):
        try:
            query = """select unit from qhc_a where code = %s"""
            result = self.db.fetch_one(query, (code,))
